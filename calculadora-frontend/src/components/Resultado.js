...
const Resultado = ({ resultadoData, onDownloadCSV }) => {
  if (!resultadoData || resultadoData.length === 0) {
    return ( <Box sx={{ p: 3, textAlign: 'center' }}><Typography variant="h5">Nenhum resultado para exibir</Typography><Typography color="text.secondary">Use o botão acima para gerar a simulação.</Typography></Box> );
  }
  const columns = [
  ...
  Object.keys(resultadoData[0]).forEach(key => { if (key.startsWith('MENSAL ANO')) { columns.push({ field: key, headerName: key.replace(/_/g, ' '), flex: 1, minWidth: 150, valueFormatter: (value) => formatarMoedaBRL(value) }); } });
  ...
  return (
    <Box>
      ...
      <Box sx={{ height: 600, width: '100%' }}>
        <DataGrid rows={resultadoData} columns={columns} getRowId={(row) => row.UNIDADE} initialState={{ pagination: { paginationModel: { pageSize: 10 } } }} pageSizeOptions={[10, 25, 50, 100]} />
      </Box>
    </Box>
  );
};
