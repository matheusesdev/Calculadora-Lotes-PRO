// Arquivo: calculadora-frontend/src/components/LoteTable.js
import React from 'react';
import { Box } from '@mui/material';
import { DataGrid } from '@mui/x-data-grid';

const LoteTable = ({ lotes, formatarMoeda, selectionModel, setSelectionModel }) => {
  const temColunaEtapa = lotes.length > 0 && lotes[0].hasOwnProperty('ETAPA');
  
  const columns = [];

  if (temColunaEtapa) {
    columns.push({ field: 'ETAPA', headerName: 'Etapa', flex: 1, minWidth: 150 });
  }
